...
router.get("/", async (req, res) => {
  try {
    ...
    // Base query with weighted relevance scoring
    let query = `
      SELECT f.id, f.filename, f.filepath, f.filetype, f.upload_date,
             m.metadata_key AS meta_key, m.metadata_value AS meta_value,
             (
               CASE WHEN f.filename LIKE ? THEN 5 ELSE 0 END +
               CASE WHEN m.metadata_key LIKE ? THEN 3 ELSE 0 END +
               CASE WHEN m.metadata_value LIKE ? THEN 2 ELSE 0 END
             ) AS relevance
      FROM files f
    ...
    const params = [
      `%${filename || ""}%`, // for filename scoring
      `%${key || ""}%`,      // for metadata key scoring
      `%${value || ""}%`     // for metadata value scoring
    ];
